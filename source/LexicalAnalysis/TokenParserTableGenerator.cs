...
namespace LexicalAnalysis
{
    public partial class TokenParserTableGenerator
    {
        ...
        public TokenParserTableGenerator(LexicalConfigurationFile configurationFile, Log? log) {
            this._log = log;
        ...
        private NFATable ConstructLexerTable() {
            this._log?.WriteLineVerbose("Constructing subtokens...");
            ...
            this._log?.WriteLineVerbose("Constructing tokens...");
            ...
            this._log?.WriteLineVerbose("Minimizing tokens...");
            var minimizedTokens = this.GetMinimizedTokens(definedTokens);

            this._log?.WriteLineVerbose("Constructing final NFA...");
        ...
        private IEnumerable<TokenNFATable> GetMinimizedTokens(Dictionary<string, TokenNFATable> definedTokens) {
            ...
            foreach (var entry in definedTokens) {
                ...
                this._log?.WriteLineVerbose($"Computing e-closure and NFA-To-DFA conversion for token {tokenName}...");
                ...
                this._log?.WriteLineVerbose($"Minimizing the dfa for token {tokenName}...");
                ...
                this._log?.WriteLineVerbose($"Removing trap states for token {tokenName}...");
        ...
        private Dictionary<string, int> GetTokenPriorities(IEnumerable<Core.Config.ConfigSection> sections) {
            ...
            foreach (var section in sections) {
                ...
                if (priorityStrs.Count() > 1) {
                    this._log?.WriteLineWarning($"Found multiple priorities for token {tokenName} in {section.GetLocation()}.");
                }
                ...
                if (!Int32.TryParse(priorityStr, out int priority)) {
                    this._log?.WriteLineError($"Unable to cast {priorityStr} to priority in {section.GetLocation()}.");
                }
        ...
        private void GetTokenDefinitions(IEnumerable<Core.Config.ConfigSection> sections, Dictionary<string, TokenNFATable> destination, Dictionary<string, TokenNFATable> support) {
            ...
            while (pendingTokens.Count != 0) {
                this._log?.WriteLineVerbose($"Starting pass {passNumber++}");
                var sectionsDefinedInThisIteration = new HashSet<Core.Config.ConfigSection>();
                foreach (var section in pendingTokens) {
                    this._log?.WriteLineVerbose($"Trying to construct {section.Header.FirstOrDefault()} in {section.GetLocation()}...");
                    if (ConstructTokenDefinition(section, destination, support)) {
                        this._log?.WriteLineVerbose($"Constructed {section.Header.FirstOrDefault()} in {section.GetLocation()}.");
                ...
                for (int componentIndex = 0; componentIndex < ruleComponents.Length; componentIndex++) {
                    ...
                    if (component.StartsWith(this.ConfigurationFile.GetRule(LexicalConfigurationFile.RULE_SUB_TOKEN_PREFIX_KEY))) {
                        ...
                        if (!support.ContainsKey(component)) {
                            this._log?.WriteLineVerbose($"Unable to define '{tokenName}' since it depends on '{component}' which has not yet been defined.");
                            return false;
                        }
    ...
    public partial class TokenParserTableGenerator : IReportable
    {
        public IEnumerable<ReportSection> GetReportSections() {
            yield return this._section;
        }
    }
}
